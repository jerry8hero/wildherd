#!/usr/bin/env python3
"""
DeepSeek 文案 Review 自动化脚本

功能：
- 读取 markdown 文案文件
- 调用 DeepSeek API 进行 review
- 根据 review 建议自动修改文案
- 支持多轮迭代

用法:
    # 单次 review（不修改）
    python3 deepseek_reviewer.py review <文件路径>

    # review + 自动修改
    python3 deepseek_reviewer.py review <文件路径> --apply

    # 多轮迭代 review + 修改（默认3轮）
    python3 deepseek_reviewer.py iterate <文件路径>

    # 自定义迭代轮数
    python3 deepseek_reviewer.py iterate <文件路径> --rounds 5

    # 设置 reviewer 类型（B站/通用）
    python3 deepseek_reviewer.py iterate <文件路径> --reviewer bilibili
"""
...
def get_api_key() -> str:
    ...
    # 从 x deepseek 配置获取
    config_path = Path.home() / ".config" / "x-cmd" / "deepseek" / "config.json"
    ...
    raise ValueError("未找到 DeepSeek API Key，请先运行 x deepseek init 或设置 DEEPSEEK_API_KEY 环境变量")


def call_deepseek(messages: list, temperature: float = 0.7, max_tokens: int = 4096) -> str:
    """调用 DeepSeek API"""
    api_key = get_api_key()
...
def review_script(file_path: str, reviewer_type: str = "bilibili", apply: bool = False) -> bool:
    ...
    """
    print(colored("\n" + "=" * 60, "cyan"))
    print(colored("🔍 DeepSeek 文案 Review", "bold"))
    ...
    try:
        ...
        if apply:
            ...
            modified_content = call_deepseek(modify_messages)
...
def iterate_review(file_path: str, rounds: int = 3, reviewer_type: str = "bilibili") -> bool:
    ...
    """
    print(colored("\n" + "=" * 60, "cyan"))
    print(colored("🔄 DeepSeek 文案迭代 Review", "bold"))
    print(colored("=" * 60, "cyan"))
    ...
    for i in range(1, rounds + 1):
        ...
        try:
            # 1. Review 当前版本
            print(colored("\n⏳ 调用 DeepSeek API 进行 review...", "yellow"))

            prompt = build_review_prompt(current_content, reviewer_type)
            feedback = call_deepseek([{"role": "user", "content": prompt}])
            ...
            if i < rounds:
                ...
                current_content = call_deepseek([{"role": "user", "content": modify_prompt}])
...
def main():
    parser = argparse.ArgumentParser(
        description="DeepSeek 文案 Review 自动化工具",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  # 单次 review（只看不改）
  python3 deepseek_reviewer.py review docs/video-scripts/鳄龟/04-鳄龟喂食指南.md

  # review + 自动修改
  python3 deepseek_reviewer.py review docs/video-scripts/鳄龟/04-鳄龟喂食指南.md --apply

  # 多轮迭代 review + 修改（默认3轮）
  python3 deepseek_reviewer.py iterate docs/video-scripts/鳄龟/04-鳄龟喂食指南.md

  # 自定义迭代轮数
  python3 deepseek_reviewer.py iterate docs/video-scripts/鳄龟/04-鳄龟喂食指南.md --rounds 5

  # 使用通用 reviewer（非B站）
  python3 deepseek_reviewer.py review docs/video-scripts/鳄龟/04-鳄龟喂食指南.md --reviewer general
"""
    )
